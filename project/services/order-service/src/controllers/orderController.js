...
// @route   GET /api/v1/orders/:id/can-review
// @access  Private
exports.canReviewOrder = catchAsync(async (req, res, next) => {
  ...
  if (order.user.toString() !== req.user._id.toString()) {
    return next(new AppError("You can only check your own orders", 403));
  }

  const canReview = order.status === "Success" && !order.isReviewed;
...
// @route   PATCH /api/v1/orders/:id/mark-reviewed
...
// @desc    Unmark order as reviewed (when review is deleted)
// @route   PATCH /api/v1/orders/:id/unmark-reviewed
// @access  Private (called by product-service)
exports.unmarkAsReviewed = catchAsync(async (req, res, next) => {
  const order = await Order.findById(req.params.id);
